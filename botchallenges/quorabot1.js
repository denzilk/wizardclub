...
function mostViewedWriters(topicIds, answerIds, views) {
    ...
    var questions = topicIds.map(function(topicgroup, i){
        return {
            question_id: i,
            topics: topicgroup,
            answers: answerIds[i].map(function(answer, j){
                return {
                    answer_id: answer,
                    user_id: viewObj[answer].user_id,
                    viewcount: viewObj[answer].viewcount
                };
            }),
            answer_ids: answerIds[i]
        }
    })

    var topics = topicIds.reduce(function(acc,val){
        for(var i= 0; i<val.length; i++){
            if (acc.indexOf(val[i])<0){
                acc.push(val[i]);
            }
        }
        return acc;
    ...
    var out = topics.map(function(topic_id){
        var userviews = {};
        var out = [];

        questions
            .filter(function(q){
               return (q.topics.indexOf(topic_id)>= 0);
            })
            .forEach(function(val){
                val.answers.forEach(function(a){
                    userviews[a.user_id] = userviews[a.user_id] || 0;
                    userviews[a.user_id] += a.viewcount;
                });
            });

        for(var i in userviews){
            out.push([Number(i), userviews[i]]);
        }
